// === Google Apps Script Web App (JSONP + Spreadsheet ID) ===
// Spreadsheet harus ada dengan sheet 'Whitelist' (header: A user_id | B angka | C flag | D nama | E created_at)
// Deploy: New deployment → Web app → Execute as Me → Who has access: Anyone
// Penting: set SPREADSHEET_ID di bawah.
...
function _sheet_(){
  ...
  if (!sh){
    ...
    sh.getRange(1,1,1,5).setValues([['user_id','angka','flag','nama','created_at']]);
  }
  return sh;
}
...
function doGet(e){
  ...
  if (values.length > 1){
    ...
    data = rows.filter(r => (r[idx.user_id] || '').trim() !== '').map(r => ({
...
function doPost(e){
  ...
  const sh = _sheet_();
  sh.appendRow([user_id, angka, flag ? 1 : 0, nama, new Date()]);
